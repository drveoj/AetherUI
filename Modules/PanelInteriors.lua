--[[--------------------------------------------------------------------------
	...
	The two were one file until this split, at which point Panels.lua was
	6,460 lines and 189 top-level locals. Lua 5.1 allows a function - and a
	chunk is a function - exactly 200, so it was ELEVEN NAMES from refusing to
	load, and the next window to be dressed would have been the one that did
	it. That is not a refactor anybody chose; it is a ceiling arriving.
	...
	nothing in here is called from Panels.lua except through that table, and
	nothing in here uses the shell's layout machinery at all.
	...
	measurements come with it: the mail row count, the who-list columns and
	the trade rows are down here now, because the shell never read them.
...
local Part, Roled, CloseButton = PN.Part, PN.Roled, PN.CloseButton
local TabAt = PN.TabAt
local Dress, DressClose, DressPager = PN.Dress, PN.DressClose, PN.DressPager
local FONT_BUMP, SKILL_HEAD, SKILL_BAR_H = PN.FONT_BUMP, PN.SKILL_HEAD, PN.SKILL_BAR_H
local MAIL_TAB_DROP = PN.MAIL_TAB_DROP

-- ---------------------------------------------------------------------------
-- Measurements the dressers use and the shell does not, moved here with them:
-- each was a name on the shell's locals list, paying rent in a chunk that had
-- run out of room.
-- ---------------------------------------------------------------------------
...
-- scrolls.
local MAIL_ROWS     = 7
-- Attachment slots on an open letter.
local MAIL_ATTACHMENTS = 16
-- Rows in the trade window, per side.
local TRADE_ROWS    = 7
-- How dark a receipt's own parchment has to be before we stop tinting over it.
local RECEIPT_DARK  = 0.5
-- Rows the client builds for each of the two lists on the friends window.
local IGNORE_ROWS   = 19
local WHO_ROWS      = 17
-- Columns down the who list.
local WHO_COLUMNS   = 5
...
local function DressCharacter(frame, store)
	...
	-- open, too, rather than once ever: Toggle set its hooked flag BEFORE it
	-- hooked, so each attempt died on the next bar that had not been reached
	...
	-- chips keep their school icons: the picture is the information, and there
	-- is no glass equivalent of "at war".
	for n = 1, (_G.NUM_FACTIONS_DISPLAYED or 0) do
