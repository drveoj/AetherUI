...
--
--  THE SHELL'S, not a dresser's, on the rule this file and its other half
--  divide by: the tab rail is something every window can have, so it lives
--  here even though the character sheet was the first window to want one.
...
-- ---------------------------------------------------------------------------
-- what Modules/PanelInteriors.lua borrows
--
-- A second file cannot see this one's locals, so everything the dressers use
-- is published here, in one place, rather than reached for one at a time. If
-- you add a borrowing, add it to this block - the alternative is a dresser
-- that silently gets nil and fails somewhere else entirely.
--
-- THE HELPERS ARE THE SHELL'S because every window gets them; the four
-- MEASUREMENTS below are here because the shell reads them too. Anything only
-- a dresser reads went with the dressers.
-- ---------------------------------------------------------------------------

PN.Roled         = Roled
PN.TabAt         = TabAt
PN.DressPager    = DressPager
